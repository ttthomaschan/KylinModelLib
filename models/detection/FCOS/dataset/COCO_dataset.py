...
from torchvision.datasets import CocoDetection  # Pytorch自带coco数据集处理模块
import numpy as np
import random
import cv2
...
class COCODataset(CocoDetection):
    ...
    def __init__(self, imgs_path, anno_path, resize=[800,1333], is_train=True, transform=None):
        super().__init__(imgs_path, anno_path)
        ids = []
        for id in self.ids:  # self.ids已经调用父类CocoDetection，内核还是调用了pycocotools
    ...
    def __getitem__(self, index):
        img, ann = super().__init__(index)
        ann = [o for o in ann if o['iscrowd'] == 0]
        boxes = [o['box'] for o in ann]
