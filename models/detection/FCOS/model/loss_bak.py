...
class GenTargets(nn.Module):
    ...
    def forward(self, inputs):
        ...
        [0]list [cls_logits,cnt_logits,reg_preds]
            cls_logits  list contains 5-Head outputs, each output shape like [batch_size,class_num,h,w]
            cnt_logits  list contains 5-Head outputs, each output shape like [batch_size,1,h,w]
            reg_preds   list contains 5-Head outputs, each output shape like [batch_size,4,h,w]
        ...
        gt_boxes = inputs[1]
        classes = inputs[2]
        '''level 表示不同Head输出的不同尺度'''
        cls_targets_all_level = []
        cnt_targets_all_level = []
        reg_targets_all_level = []
        assert len(self.strides) == len(cls_logits)  ## 输入的stride也为list，且数量和Head相同
        for level in range(len(cls_logits)):
            '''level_out为同一层Head的三个输出：cls、cnt、reg'''
    ...
    def _gen_level_targets(self, out, gt_boxes, classes, stride, limit_range, sample_radiu_ratio=1.5):
        """
        Args
        out： list contains 1-Head output， shape like -  [[batch_size,class_num,h,w], [batch_size,1,h,w], [batch_size,4,h,w]]
...
def compute_cnt_loss(preds, targets, mask):
    ...
    for batch_index in range(batch_size):
        ...
        loss.append(
            ...
            # nn.BCEWithLogitsLoss(input=pred_pos, target=target_pos, reduction='sum').view(1))
    return torch.cat(loss, dim=0) / num_pos  # [batch_size,]
