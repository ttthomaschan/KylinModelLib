...
if __name__ == "__main__":
    ...
    class Config():
        ...
        limit_range = [[-1, 64], [64, 128], [128, 256], [256, 512], [521,9999]]

    model = FCOSDetector(mode='inference', config=Config)
    model = torch.nn.DataParallel(model)
    model.load_state_dict(torch.load('./checkpoint/model_17.pth', map_location=torch.device('cpu')))
    ...
    for name in names:
        ...
        img_pad = preprocess_img(img_bgr, [1500, 2000])
        img = cv2.cvtColor(img_pad.copy(), cv2.COLOR_BGR2RGB)
        img1 = transforms.ToTensor()(img)
        img1 = transforms.Normalize([], [], inplace=True)(img1)
        img1 = img1.to('cuda')
        ...
        for i, box in enumerate(boxes):
            ...
            bbox = patches.Rectangle((box[0], box[1]), width=box[2] - box[0], height=box[3] - box[1],
                                     linewidth=1, facecolor='none', edgecolor=b_color)
            ax.add_patch(bbox)
            plt.text(box[0], box[1],
                     s="{} {}".format("table", scores[i]),
                     color='white',
